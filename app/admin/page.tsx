...
export default function AdminDashboard() {
  ...
  const getActivityIcon = (iconName:any) => { // add any
    const iconMap = {
      Users: Users,
      BookOpen: BookOpen,
      UserCog: UserCog,
  ...
  const recentActivities =
    dashboardData?.recentActivities?.map((activity:any) => ({ // add any
      ...activity,
        ...
        <Tabs defaultValue="overview" className="space-y-4">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value"
                      >
                        {userDistributionData.map((entry:any, index:any) => ( // add any
                ...
                <CardContent>
                  <div className="space-y-4">
                    {recentActivities.length > 0 ? (
                      recentActivities.map((activity:any) => ( // add any
                        <div
                          key={activity.id}
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentUsers.length > 0 ? (
                    recentUsers.map((user:any) => ( // add any
                      <div
                        key={user.id}
          ...
          <TabsContent value="performance" className="space-y-4">
              ...
              <CardContent>
                <div className="space-y-4">
                  {dashboardData?.performanceData
                    ?.filter((perf:any) => perf.avg > 0) // add any
                    .map((performance:any, index:any) => ( // add any
                      <div
                        key={index}
                        className="flex items-center justify-between p-4 rounded-lg border bg-card"
                      >
                  ...
                  {(!dashboardData?.performanceData ||
                    dashboardData.performanceData.filter((p:any) => p.avg > 0) // add any
                      .length === 0) && (
                    <p className="text-muted-foreground text-center py-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {dashboardData?.topPerformers?.map((performer:any, index:any) => ( // add any
                    <div
                      key={index}
                      className="flex items-center justify-between p-4 rounded-lg border bg-card"
                    >
